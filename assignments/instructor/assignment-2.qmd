...
Due date is <span style='color:blue'>{{assignment-2-due|%B %e}}</span> <span style='color:red'>before</span> the lab session on that day, in which this assignment is reviewed by the TA. Submission after the deadline would result in a heavy discount on the score. 

Please submit your assignment (both the qmd and the resulting html files) using the following [submission link]({{submission}}). Name your qmd file using the following convention: `lastname_assignment_2.qmd`. For example, it would be `Mieno_assignment_2.qmd` for me. 
