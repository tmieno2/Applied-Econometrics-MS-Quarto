...
<!-- DO NOT EDIT. Generated from assignment-2.qmd (md5:df0e6173e5d6e75234a33143e9f39fcf) by assignments/strip-answers.R -- edit the instructor source instead. -->
...
Due date is <span style='color:blue'>November 6</span> <span style='color:red'>before</span> the lab session on that day, in which this assignment is reviewed by the TA. Submission after the deadline would result in a heavy discount on the score. 
...
::: {.answer}
negative because more hours at work means less time to sleep
:::
...
::: {.answer}
One is expected to sleep `r 0.148*5*60` (in minutes) less on average. For a week, this does not seem a large trade-off to me.
:::
...
::: {.answer}
More education implies less predicted time sleeping, but the effect is quite small. If we assume the difference between college and high school is four years, the college graduate sleeps about `r 11.13*4` minutes less per week, other things equal.
:::
...
::: {.answer}
$R^2$ is only $0.113$, which indicates there are other factors than $totwrk$, $educ$, and $age$ that should explain the remaining variation in $sleep$. Variables that are missing (omitted) in the models would be health status, marital status, the number of kids. They all seem to be correlated with $totwrk$. Consequently, the coefficient estimator on $totwrk$ in the above equation is likely to be biased.
:::
...
beta_tilde_1
beta_hat["educ"] + beta_hat["IQ"] * sigma_tilde_1

#--- never compare doubles with `==`; they differ by rounding error ---#
all.equal(
  unname(beta_tilde_1),
  unname(beta_hat["educ"] + beta_hat["IQ"] * sigma_tilde_1)
)
```
------
...
::: {.answer}
If the proportion of black population goes up by $0.1$, then the price of soda goes up by \$`r round(0.1 * coef(reg_c8)["prpblck"], digits = 4)`. Given the average soda price is `r round(mean(data_c8$psoda),digits=3)`, I would not consider this impact as economically significant.
:::
...
::: {.answer}
The simple regression puts the coefficient on $prpblck$ at
`r round(coef(reg_c8_simple)["prpblck"], digits = 3)`, against
`r round(coef(reg_c8)["prpblck"], digits = 3)` once $income$ is controlled for. So the
estimated discrimination effect is **larger**, not smaller, when we control for income.

That is what the omitted variable bias formula predicts here. $income$ raises $psoda$
($\hat{\beta}_2>0$) and $income$ is negatively correlated with $prpblck$
(`r round(cor(data_c8$prpblck, data_c8$income), digits = 3)`), so the bias term
$\hat{\beta}_2\tilde{\sigma}_1$ is **negative**: leaving $income$ out pulls the
coefficient on $prpblck$ *down*, and putting it back in raises the estimate.
:::
...
::: {.answer}
The estimated percentage change in $psoda$ induced by .20 increase in $prpblck$ is $0.2\times$ `r round(coef(reg_c8_4)["prpblck"], digits = 3)` $\times 100=$ `r round(100 * 0.2 * coef(reg_c8_4)["prpblck"], digits = 2)`$\%$.
:::
...
```{r reg_c8_5}
reg_c8_5 <- feols(log(psoda) ~ prpblck + log(income) + prppov, data = data_c8)
msummary(
  reg_c8_5,
  # keep these options as they are
  stars = TRUE,
...
```{r reg_c9_3}
reg_c9_3 <- feols(log(psoda) ~ prpblck + log(income) + prppov + log(hseval), data = data_c8)
msummary(
  reg_c9_3,
  # keep these options as they are
  stars = TRUE,
...
::: {.answer}
A one **percent** increase in housing value would increase the price of soda by `r round(coef(reg_c9_3)["log(hseval)"], digits = 3)`$\%$ on average. The two sided $p$-value for $H_0$: $\beta_{log(hseval)}=0$ is the following:
:::

```{r p_c9_3}
tidy(reg_c9_3) %>%
  filter(term == "log(hseval)") %>%
  pull(p.value)
```
...
```{r reg_c9_4}
linearHypothesis(reg_c9_3, c("log(income)", "prppov"), test = "F")
```
<br>
