...
That second outcome is the counterfactual, and it is missing. For example, after a Nebraska well is regulated, you observe its regulated pumping, but you cannot observe that same well, in the same weather and year, pumping without the limit. You therefore need another observation or group to stand in for the missing outcome.

Now look at the key-challenge callout. Most programs of interest are not randomized. Farmers choose whether to adopt technology or insurance, workers choose whether to enter training, and eligibility rules direct assistance toward people who already differ from nonrecipients. That self-selection makes program status correlated with other determinants of the outcome, which is the endogeneity problem named at the bottom of the callout. A raw participant-versus-nonparticipant comparison will then mix the program's effect with those prior differences. Difference-in-differences, or DID, is one way to construct a more credible counterfactual when one group becomes exposed and another does not. The rest of the lecture asks exactly what DID removes and what assumption it still requires.
...
A credible natural experiment has a source of variation that is plausibly unrelated to untreated potential outcomes after accounting for the design. That credibility comes from institutional facts and observable evidence, not from the label. DID is often paired with natural experiments because it compares an exposed group with an unexposed group before and after the event. The method can remove fixed group differences and shared time shocks, but it cannot rescue an assignment process that produces different untreated trends. That distinction sets up the three approaches in the next section.
...
We will use the incinerator example for all three approaches so the target effect stays fixed. Approach one is a post-treatment cross-section: compare nearby treated houses with farther-away controls in 1981. Approach two is a before-after comparison: compare nearby houses in 1978 with nearby houses in 1981. Approach three, DID, combines the group comparison with the time comparison. Each step removes a different source of confounding. The post-only comparison retains both any baseline location gap and any difference in untreated changes. The treated before-after comparison removes a fixed location gap but retains every other time change. DID removes fixed group differences and time changes shared by both groups, but it still requires the groups to have had the same change without treatment.
...
The data contain sale prices and other house characteristics in both 1978 and 1981. We will call 1978 the before period because physical construction had not begun, and 1981 the after period because construction had begun. The outcome is the real, inflation-corrected sale price. If the incinerator makes nearby locations less desirable, the causal effect we seek should appear as a less favorable price change for nearby houses.

The rumor date is not a minor historical detail. It raises an anticipation concern. Buyers and sellers may capitalize expected future harm into prices before construction begins. If rumors already lowered nearby prices during 1978, then that year's price is not a completely untreated baseline. The resulting DID may miss an earlier part of the effect. We will leave the slide's before-and-after classification in place for the classroom calculation, but a real evaluation would investigate the announcement process, exact sale dates, and earlier price trends. This example will let us compare the mechanics of three estimators while also seeing why institutional timing matters.
...
Two indicator variables will do the main design work. Near-inc equals one for houses near the site and zero for houses farther away. Why eighty-one equals one for 1981 sales and zero for 1978 sales. Later, their product will equal one only for nearby houses in 1981, the treated group in the after period.

Notice also what the rows represent. This is a repeated cross-section, not necessarily a panel of the same houses. We observe sales from each location group in each year, but a house sold in 1978 need not be sold again in 1981. DID can work with repeated cross-sections if each cell represents the relevant population consistently. Changes in which kinds of houses sell, however, can create a composition problem, which is why the additional house characteristics can matter in a fuller analysis.
...
These three callouts are our roadmap. Approach one uses only the after period, 1981. It compares the mean real price of houses close to the incinerator with the mean for houses farther away. Its implicit counterfactual says the far-house price level in 1981 tells us what nearby houses would have cost in 1981 without construction. That requires the locations to be comparable not only in their change but also in their starting level.

Approach two uses only the treated location group but both years. It compares nearby-house prices before and after construction began. Its implicit counterfactual says the nearby group's 1978 mean tells us what its 1981 mean would have been without the incinerator. That requires no other force to have changed nearby real prices between the years.

Approach three combines both dimensions. First find the 1978-to-1981 change for nearby houses. Then find the same change for far houses. Finally subtract the second change from the first. The far group's change is used as the counterfactual time change for the near group. This difference-in-differences approach allows the groups to start at different levels and allows prices to change over time for reasons unrelated to construction. It requires those untreated changes to be the same across groups. As you move through the tabs, keep identifying the missing outcome and the observed substitute used for it.
...
Even if you add observed housing controls, unobserved location differences may remain. The central mistake would be to interpret an after-period group difference as a treatment effect merely because the treatment is geographically defined. We first need to distinguish the mechanical meaning of delta one from its causal interpretation, which is what the next tab does.
...
This result follows mechanically from a regression with an intercept and one binary regressor. It is a descriptive identity, not yet a causal result. Delta one can contain a price difference that existed before construction, a difference in how the two locations would have changed for other reasons, and the incinerator's effect. Statistical estimation tells us the size and uncertainty of the observed gap. Identification tells us whether that gap isolates the causal parameter. Keep that distinction in mind when we run the regression on the next tab.
...
That is the critical fact in the blue callout. The 1981 gap of negative 30.69 did not begin from zero. At least negative 18.82 thousand dollars of near-versus-far difference was already visible in 1978. It may reflect stable neighborhood and house differences. Because rumors had begun, it might also include anticipation, which would mean 1978 is not fully untreated. Either explanation prevents us from assigning the full 1981 gap to construction. A post-only design would need equal untreated levels and equal untreated changes. The observed baseline gap directly contradicts the equal-level part. The visualization next puts all four group-year means together.
...
Now read the graph across time within each group. Far-house prices rise by about 18.79 thousand dollars, from 82.52 to 101.31. Near-house prices rise by only about 6.93, from 63.69 to 70.62. Approach two uses only that 6.93 increase. DID compares the increases: 6.93 minus 18.79 equals about negative 11.86 thousand dollars. Visually, that is the amount by which the near group's change falls short of the far group's change. The graph cannot establish parallel trends with only one before year, but it does show exactly why DID removes the starting-level gap while a post-only comparison does not.
...
This strategy avoids attributing a fixed near-versus-far level difference to treatment because it does not use the far group at all. That is an advantage over Approach one. But the error term now contains every other force that changed nearby prices over time. Regional housing demand, interest rates, local employment, public services, neighborhood development, and changes in which kinds of nearby houses were sold can all move the 1981 mean. Inflation correction removes general price inflation, but not real housing-market changes. For beta one to equal the incinerator effect, nearby-house prices would have had to remain unchanged between 1978 and 1981 in the absence of construction. That is the strong identifying condition we will expose algebraically.
...
This coefficient is descriptive until we supply a counterfactual assumption. In the notation used later, the 1981 nearby mean includes both the change from non-incinerator causes and the incinerator's causal effect. The 1978 nearby mean by itself tells us the starting point, but not where that group would have gone without construction. The calendar sequence, construction happened between the two samples, does not prove that construction caused the full change. Treating every before-after movement as a program effect is the post hoc mistake this approach illustrates. On the next tab, the fitted coefficient shows why even the direction of the raw change can be misleading about the direction of the treatment effect.
...
Using our mean notation, this coefficient estimates alpha one plus beta. Alpha one is what nearby prices would have changed because of all other forces, while beta is the construction effect. With only the treated group's two means, those components cannot be separated. We need an observed estimate of the untreated market movement, which is the role the far group will play in DID.
...
DID replaces that zero-change assumption with a comparative one. It uses the observed change for far houses, alpha zero, as an estimate of the missing alpha one. If alpha one and alpha zero are equal, subtracting the far change removes the untreated movement. Move to Approach three to see that second subtraction.
...
That interpretation shows how the regression performs DID. The treated main effect absorbs the fixed baseline location gap, the time main effect absorbs the change shared with controls, and the interaction measures what remains. Calling beta three an effect still requires parallel untreated trends and the other design assumptions. The next tab verifies the difference-of-differences interpretation for every combination of the two indicators.
...
With several pre-treatment periods, you can compare earlier trends. Divergence before treatment is evidence against the assumption. Similar pre-trends are reassuring but cannot prove that no group-specific shock would have emerged after treatment. You should also use institutional knowledge to investigate anticipation, concurrent local events, spillovers, and sample composition. Parallel trends becomes credible through a coherent design argument supported by diagnostics, not through one insignificant pre-trend test.
...
DID is attractive because equal untreated changes can be more plausible than equal levels or zero change, especially with a carefully chosen control group. Whether it is plausible is a substantive question. The next historical example shows how institutional context can support that comparison.
...
This historical example concerns cholera in mid-nineteenth-century London. The first two bullets contrast the leading explanations. Many people believed cholera spread through contaminated air, the miasma theory. John Snow argued instead that transmission occurred through water contaminated with fecal matter. To evaluate that claim, we need variation in exposure to polluted water that is not simply a comparison of richer and poorer neighborhoods.
...
That change creates the natural experiment. Access to Lambeth water became access to a cleaner source, while the other suppliers remained a comparison. The before period is the 1849 epidemic, when Lambeth still drew downstream. The after period is 1854, after Lambeth moved upstream. The outcome will be cholera deaths per 10,000 people.

The DID design asks whether mortality changed more favorably in districts with Lambeth access than in districts served only by Southwark and Vauxhall. The regression arithmetic will be simple. Its causal force comes from the historical supply arrangement and from whether the two sets of districts would otherwise have had comparable mortality changes.
...
The control group's mortality change is supposed to capture epidemic-wide changes between 1849 and 1854, such as weather, population conditions, or the overall severity of cholera. The treated group's change contains those shared forces plus the effect of cleaner water. Parallel trends says that without Lambeth's upstream move, mortality in the two district groups would have changed by the same amount.

For that claim to be credible, the groups should not experience other differential changes tied to cholera risk. Household sorting or a selective change in supplier composition could also contaminate the comparison. The next tab computes the DID, but keep those design conditions attached to the number.
...
The printed data also include p-r, precipitation, and e-t-zero, reference evapotranspiration. Those weather variables help control for changing water demand. Available water capacity and bulk density are soil characteristics, but they are not used directly in the displayed regressions. Because the same site can appear in many years, later specifications can use site fixed effects and allow regression errors to be correlated within a well over time.

Keep the treatment level in mind, however. The policy is assigned to one geographic region, not independently to 201 unrelated wells. Well-level clustering addresses serial correlation, but common regional or spatial shocks can still make conventional inference too optimistic. We will return to that issue after building the basic DID variables.
...
The map also tells you what to worry about. Control wells are geographically dispersed around the treated region, so farms inside and outside the boundary may differ in soils, crops, aquifer conditions, or prior pumping trends. Site fixed effects can absorb stable differences, but not different changes over time. Spillovers are also possible. Reduced pumping inside the boundary can affect groundwater conditions outside it, or producers can shift activity across the line. If outside wells are indirectly affected, their post-policy trend is no longer an untreated counterfactual. The map is therefore part of the design argument, not just an illustration. Next we encode the 2013 timing in the data.
...
Treatment-group membership does not need to be created because in-L-E-M-A already equals one for wells inside the boundary and zero outside. The DID interaction is the product of before-after and in-L-E-M-A. It equals one only for inside wells from 2013 onward. It is zero for treated wells before the policy and for control wells in every year.

Do not stop after seeing one well coded correctly. In an applied project, verify that the policy date is correct for all observations, that in-L-E-M-A is stable within site, that years are not duplicated, and that missingness or reporting rules did not change at treatment. A perfectly formed interaction can still estimate a compositional break instead of a policy effect if the underlying sample changes.
...
The post-treatment separation therefore cannot be interpreted by simply applying a common-trend DID. A treatment effect is added in the simulation, but the graph does not let a basic DID cleanly separate that effect from the pre-existing differential trend. Possible responses include finding a control group with a more credible trajectory, choosing a substantively justified time window, modeling a trend structure that theory supports, or selecting a different research design. Adding a linear group trend mechanically can also extrapolate poorly, so it needs justification. The key response is not to ignore the pre-period divergence or to focus only on whether a formal pre-trend test happens to reject.
...
Do not choose a starting year only because deleting inconvenient early observations makes the lines look parallel. A defensible window should follow from data quality, policy history, agronomic conditions, or the comparison you intend to estimate. Report sensitivity to alternative windows. Finally, these are group averages. They can hide heterogeneous well paths, changing sample composition, and spatial shocks, so pair the graph with design knowledge and formal diagnostics.
...
Choose a planned set of substantively meaningful placebo dates, report point estimates and confidence intervals, and consider a joint test where appropriate. Interpret the placebos alongside the plotted trajectories and institutional history. The next three tabs show how different fake dates can reveal different parts of the LEMA pre-period.
...
Do not summarize the two tabs only as two passes. Compare the magnitudes and confidence intervals, relate each split to the trajectory graph, and consider the limited number of early baseline years. If placebo conclusions change sharply when the date or sample window moves, that instability belongs in your evaluation of the real 2013 estimate. The final tab chooses an earlier sample and exposes a much clearer warning.
...
Do not subtract negative 16.39 from the 2013 estimate. A placebo at another date does not directly measure the actual coefficient's bias. It warns that the counterfactual comparison is sensitive and that a correctly coded fixed-effect regression can still have an implausible design. The result should prompt investigation of the early years, sensitivity to justified windows, and alternative controls, not a mechanical bias correction.
...
Many real policies do not have common timing. States pass laws in different years, counties enter programs in waves, and firms adopt technologies gradually. That is staggered adoption. In that setting, running the same-looking two-way fixed-effects regression still produces a coefficient, but the coefficient need not be the simple average treatment effect you intend. The next tab explains where the problematic comparisons enter.
...
This is not a software bug, and adding more unit and year fixed effects does not solve it. The regression is answering a different weighted-comparison question. The problem disappears under strong homogeneous-effect conditions, but those conditions are often least plausible when effects build, fade, or differ by cohort. Before choosing an estimator, you must therefore identify whether adoption timing is common or staggered.
...
For any of these approaches, you still need the substantive DID assumptions: credible comparison groups, parallel untreated trends for the comparisons actually used, no anticipation, and well-defined treatment without problematic spillovers. A modern estimator fixes the weighting and comparison problem; it cannot fix a bad research design.

The scope callout sets your responsibility. You do not need to derive these estimators in this course. You do need to recognize staggered adoption, name the estimator you used, and explain why its comparison groups are appropriate. That timing check is the final habit to carry from this lecture into your own impact-evaluation project.
