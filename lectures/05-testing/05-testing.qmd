...
4. The actual difference you observed is 10.2. What is the probability that you observe a number greater than 10.2 if the null hypothesis is true? Use `pnorm()`, which gives the probability that a standard Normal draw is below the value you hand it.
...
Read the callout at the bottom. A critical value is simply a quantile of the null distribution. The R function `qnorm(p)` returns the number below which a standard Normal draw falls with probability p, so `qnorm(0.95)` is about one point six four: ninety-five percent of standard Normal draws are smaller than that number and five percent are larger. Here we use `qt` instead of `qnorm` because estimating sigma puts the statistic on a t distribution, and `qt` also requires its degrees of freedom.
...
:::{.callout-note title="Critical values are just quantiles"}
`qnorm(p)` gives you the number $q$ such that a single standard Normal draw is below $q$ with probability $p$. So `qnorm(0.95) = 1.64`: ninety-five percent of standard Normal draws are smaller than 1.64, and five percent are larger.

That is all a <span style = "color: blue;">critical value</span> is. The only change below is which distribution you ask: `qt()` instead of `qnorm()`, because we had to estimate $\sigma$ rather than know it, so `qt()` also needs the degrees of freedom.
:::
