...
You combine a prefix with a family name: dee-beta, pee-you-nif, cue-chi-squared, or R-low-jiss, for example.
...
The crucial warning is that a density height is not itself a probability. For a continuous variable, the probability of drawing exactly one specified value is zero. Probabilities come from areas under the density over intervals.
...
Pee-norm of minus one accumulates every thin strip of area from the far left tail through minus one, giving about zero point one five nine. Density height and cumulative area answer different questions.
...
Open and run the answer cell to confirm. Read the code as “pee-norm of one, with mean one and S-D two.” Each argument is doing the job we just identified. The answer would remain zero point five for any positive standard deviation because changing the spread does not move the center.
...
As with the other Normal functions, the omitted defaults are mean zero and standard deviation one. So cue-norm of x by itself returns a standard Normal quantile. Keep the direction straight: pee goes from value to probability; cue goes from probability to value.
...
Open the answer and run the direct call to confirm.
