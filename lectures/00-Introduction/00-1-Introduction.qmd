...
This is the most important slide in the lecture, so let's slow down. Look at the four arrow diagrams. A affects B. B affects A. They affect each other. And in the last one, C affects both A and B, with no arrow between A and B at all. In every case, A and B may move together. A correlation coefficient cannot tell these causal structures apart; it only measures their association. If our question is the causal effect of A on B, only the first and third diagrams contain that arrow, and the third adds reverse causality. Now read the note about the fourth case: A and B can be associated even though neither one affects the other. That case is the source of nearly every headache in this course.
...
Look at the two bullets, because they say why an experiment is the gold standard. If you get to set the variable of interest yourself, at random, you break its link with everything in the error term by construction. The caveat in blue is worth taking seriously: that holds on average over repeated randomizations, so any one small experiment can still get unlucky. The tabs that follow are the contrast: a field where nobody randomized anything, and then the same field if someone had.
...
The map on screen is the opposite of the previous tab. This is nitrogen application on a real field, chosen by the farmer. See how the rates come in large uniform blocks rather than scattered around? That pattern is a decision, and in the next tab we ask what the decision was based on.
...
So why not do that everywhere? Read the first question and take it seriously: would you randomly assign how many years of education people get? In most settings you cannot, and you should not. That generalizes into a deep reason empirical economics is hard, which is the second block on the screen. We study human behaviour, and people choose using the information they have, aiming at outcomes they want: how much school, how many firefighters, how much nitrogen. Those purposeful decisions often create a correlation between the choice and something we cannot see. If economically important choices were routinely randomized, causal effects would be much easier to identify.
